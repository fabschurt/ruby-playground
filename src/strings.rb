require 'minitest/autorun'

describe 'A `String` object' do
  it 'can be formatted with the `%` syntax' do
    variables = ['Jack', 32, 'engineer']

    _('Hi, I’m %s, I’m a %d-year-old %s.' % variables).must_equal 'Hi, I’m Jack, I’m a 32-year-old engineer.'
  end

  it 'can be formatted with `Kernel#format` or `Kernel#sprintf`' do
    variables = ['pie', Math::PI]

    _(
      sprintf(
        'Do you like apple %s? No, I prefer apple %.2f.',
        *variables
      )
    ).must_equal 'Do you like apple pie? No, I prefer apple 3.14.'

    _(method(:sprintf)).must_equal method(:format)
  end

  it 'can be formatted with named placeholders' do
    vars = {name: 'Robert', age: 54.556}

    _('Hi, I’m %{name}, and I’m %{age}-year old.' % vars).must_equal 'Hi, I’m Robert, and I’m 54.556-year old.'
    _(sprintf('%<name>s is my name, and %<age>.1f is my age', vars)).must_equal 'Robert is my name, and 54.6 is my age'
  end

  it 'can be padded' do
    words = %w(earth wind fire transparent_liquid_stuff)
    max_length = words.map { |word| word.length }.max

    _{
      puts words
        .map { |word| word.rjust(max_length, '-') }
        .join("\n")
    }.must_output <<~STR
      -------------------earth
      --------------------wind
      --------------------fire
      transparent_liquid_stuff
    STR
  end
end
